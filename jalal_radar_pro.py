...
def sahmk_get(path):
    ...
    if not key: return {"error": "مفتاح سهمك غير موجود"}
    try:
        url = SAHMK_BASE + path
...
def sahmk_price(symbol):
    ...
    try:
        return float(r.get("price") or r.get("last_price") or r.get("close") or 0)
    except: return None

def sahmk_history(symbol, period="1y"):
    """بيانات تاريخية لسهم تاسي"""
...
# أسهم تاسي الحلال الرئيسية
SA_STOCKS = {
    "2222": "أرامكو السعودية",
    "1180": "الأهلي السعودي",
    "1120": "الراجحي",
    "2010": "سابك",
    "2020": "سابك للبتروكيماويات",
    "1030": "الرياض",
    "2350": "المتطورة",
    "4200": "الاتصالات السعودية",
    "7010": "stc",
    "2380": "بترو رابغ",
    "1211": "معادن",
    "3010": "نمي",
...
# ════════════════════════════════════════════════════════════
# مسح تاسي — عبر سهمك API + تحليل JRF
# ════════════════════════════════════════════════════════════
def run_scan_sa():
    """مسح أسهم تاسي — البيانات من سهمك، التحليل JRF"""
    import pandas as pd
    market = "sa"
    scan_state[market]["status"] = "scanning"
    scan_state[market]["progress"] = 0

    items = list(SA_STOCKS.items())
    total = len(items)
    results = []
    lock = threading.Lock()
    done = [0]
    ...
    def scan_one_sa(code, name):
        try:
            # السعر اللحظي من سهمك
            live_price = sahmk_price(code)

            # البيانات التاريخية من سهمك
            df = sahmk_history(code)

            # لو ما طلعت بيانات سهمك، نجرب yfinance (رمز تداول + .SR)
            ...
            # تحليل JRF (نفس المنطق لكن بدون فلتر السعر الدولاري)
...
def analyze_symbol_sa(df, code, name, price):
    """تحليل JRF للأسهم السعودية — بالريال"""
    try:
        c = df.copy()
        # نفس مؤشرات JRF
        ...
        def _macd(s):
            ...
            sg=ml.ewm(9,adjust=False).mean(); return ml,sg,ml-sg
        ...
        macd_sig = float(sg.iloc[-1])
        ema20 = float(e20.iloc[-1])
        ema50 = float(e50.iloc[-1])
        ema200 = float(e200.iloc[-1])
        vol_now = float(c["Volume"].iloc[-1])
        vol_avg = float(c["Volume"].rolling(20).mean().iloc[-1]) or 1
        ...
        # المستوى
        if score >= 15: e_tier = 1
        elif score >= 11: e_tier = 2
        else: e_tier = 3
        ...
        return {
            ...
            "verdict": verdict, "rr": rr,
            "t1": t1, "t2": t2, "sl": sl,
            "rsi": round(rsi,1), "macd": round(macd_val,4),
            "ema20": round(ema20,2), "ema50": round(ema50,2),
            "vr": vr, "entry_tier": e_tier,
            "is_crypto": False, "is_sa": True,
            "lb": round(price*0.99,2),
        }
    except: return None
    ...
    <div class="sub-body">
      ...
      <div id="sub-settings" class="hidden">
        ...
        <div class="field" style="margin-top:11px;">
          <label>🇸🇦 مفتاح سهمك API (للتاسي)</label>
          <input class="input" id="c-sahmk" type="password" placeholder="shmk_test_…">
        </div>
...
function setMarket(m) {
  ...
  var labels = {'us':'🔍 مسح السوق الأمريكي','crypto':'🔍 مسح الكريبتو','sa':'🔍 مسح تاسي 🇸🇦'};
  document.getElementById('scan-btn').innerHTML = labels[m] || '🔍 مسح';
  loadResults();
  loadFavorites();
}
