...
import warnings, threading, os, json, time
...
def _safe_float(v, default=0.0):
    """تحويل آمن لـ float بدون استثناء"""
    try:
...
def load_cfg():
    ...
    defaults = {
        ...
        "daily_loss_pct": 3.0,
        "min_confidence": 70,
        "position_pct": 2.0,        # % من رأس المال لكل مركز
        "auto_buy": False,
        "auto_sell": True,
...
def api_last_price(symbol, is_crypto=False):
    ...
    if is_crypto:
        ...
        try:
            p = float(r["trade"]["p"])
            # تحقّق بسيط: الأسهم الأمريكية نادراً تتجاوز $10,000 (باستثناء BRK.A)
            if p <= 0 or (p > 10000 and symbol not in ("BRK.A", "NVR", "BKNG")):
                return None
            return p
        except: return None
...
def analyze_symbol(code, name, is_crypto=False, live_price=None):
    ...
    try:
        ...
        # تحقّق: لو السعر اللحظي يختلف عن yfinance بأكثر من 20% → نرفضه (بيانات خاطئة)
        if live_price and yf_price > 0:
            diff_pct = abs(live_price - yf_price) / yf_price * 100
            price = live_price if diff_pct <= 20 else yf_price
        else:
        ...
        # الوقت المتوقع للوصول للهدف 1 (مبني على ATR والاتجاه)
        # متوسط الحركة اليومية = ATR، المسافة للهدف ÷ الحركة اليومية = أيام تقريبية
        daily_move = atr_v if atr_v > 0 else price*0.015
        dist_to_t1 = abs(t1 - price)
        raw_days = max(1, round(dist_to_t1 / daily_move))
        # نعدّل حسب قوة الاتجاه (ADX عالي = أسرع)
        ...
        elif est_days <= 10: eta = f"{est_days}-{est_days+3} أيام"
...
def safe_buy(signal, source="auto"):
    """
    شراء آمن — رأس المال يحكم:
    1. السوق مفتوح؟
    2. ما عندنا نفس السهم مفتوح أو مشترى اليوم؟
    3. المراكز المفتوحة أقل من الحد الأقصى؟
    4. ما تجاوزنا حد الخسارة اليومي؟
    5. السعر منطقي؟ (فلتر 15%)
    6. الكاش كافي؟
    """
    cfg = load_cfg()
    ...
    # 2a. ما عندنا نفس السهم مفتوح حالياً؟
    open_positions = api_positions()
    open_symbols = [p.get("symbol","").replace("/USD","-USD") for p in open_positions]
    if code in open_symbols:
        return {"ok": False, "msg": f"{code} عندنا مركز مفتوح فيه"}

    # 2b. ما اشترينا نفس السهم اليوم؟
    ...
    if any(t.get("symbol")==code for t in today_buys):
        return {"ok": False, "msg": f"اشترينا {code} اليوم مسبقاً"}

    # 3. المراكز المفتوحة أقل من الحد؟
    max_open = int(cfg.get("max_daily_trades", 5))
    if source == "auto" and len(open_positions) >= max_open:
        return {"ok": False, "msg": f"المراكز المفتوحة وصلت الحد ({max_open}) — ننتظر بيع"}

    # 4. حد الخسارة اليومي
    if check_daily_loss():
        return {"ok": False, "msg": "تجاوزنا حد الخسارة اليومي — التداول متوقف"}

    # 5. السعر اللحظي + فلتر التحقّق
    ...
    if signal_price > 0 and price > 0:
        diff_pct = abs(price - signal_price) / signal_price * 100
        if diff_pct > 15:
            return {"ok": False, "msg": f"تباين كبير في السعر ({diff_pct:.1f}%) — يُحتمل خطأ في البيانات"}

    # 6. حجم المركز = 2% من رأس المال (أو max_position_usd لو أصغر)
    acc = api_account()
    equity = float(acc.get("equity", 0)) if "error" not in acc else 0
    cash   = float(acc.get("cash", 0))   if "error" not in acc else 0
    pos_pct  = float(cfg.get("position_pct", 2.0))
    max_usd  = float(cfg.get("max_position_usd", 2000))
    size_usd = min(max_usd, round(equity * pos_pct / 100, 2)) if equity > 0 else max_usd

    if cash < size_usd:
        return {"ok": False, "msg": f"الكاش غير كافٍ (${cash:.0f} < ${size_usd:.0f})"}
    ...
    result = api_place_order(code, qty, "buy", "limit", signal["lb"], tif, is_crypto)
    if "error" in result:
    ...
    add_trade({
        ...
        "limit_price":signal["lb"], "tp1":signal["t1"], "tp2":signal["t2"],
    ...
    return {"ok": True, "qty": qty, "order_id": order_id, "price": price, "size_usd": size_usd}
...
def verify_pending_orders():
    ...
    - أوامر من يوم سابق → يلغيها
    - أوامر معلّقة أكثر من 4 ساعات في نفس اليوم → يلغيها (السعر ما وصل)
    """
    cfg = load_cfg()
    ...
    for o in open_orders:
        ...
        created_day = created_str[:10] if created_str else ""
        oid = o.get("id","")
        if not oid: continue
        reason = None
        # يوم سابق
        if created_day and created_day < today:
            reason = "أمر معلّق من يوم سابق — أُلغي"
        # نفس اليوم لكن أكثر من 4 ساعات
        elif created_str:
            try:
                created_dt = datetime.fromisoformat(created_str.replace("Z","+00:00"))
                hours_old = (now - created_dt).total_seconds() / 3600
                if hours_old > 4:
                    reason = f"أمر معلّق {hours_old:.1f} ساعة — أُلغي تلقائياً"
            except: pass
        if reason:
            api_cancel_order(oid)
            add_trade({
                "symbol":o.get("symbol",""), "side":"cancel",
                "qty":o.get("qty","0"), "reason": reason,
...
def monitor_positions():
    ...
    for pos in positions:
        ...
        if matching and matching[-1].get("sl"):
            ...
            # مركز يدوي بدون sl → نحسب وقف افتراضي 3% تحت الدخول
        ...
        if not st["sold_tp1"] and pnl_pct >= tp1_pct:
            q = round(st["orig_qty"]*0.40,6) if is_crypto else max(1,int(st["orig_qty"]*0.40))
        ...
        # ── المرحلة 2: +tp2% بيع 30% ──
            ...
            q = round(st["orig_qty"]*0.30,6) if is_crypto else max(1,int(st["orig_qty"]*0.30))
            q = min(q, qty)
            r = api_place_order(norm, q, "sell", "market",
                                tif=("gtc" if is_crypto else "day"), is_crypto=is_crypto)
            if "error" not in r:
        ...
        if st["sold_tp1"] and not sold:
            trail = st["highest"]*(1-trail_pct/100)
            if current <= trail:
        ...
        # ── وقف الخسارة الأصلي (قبل أي بيع) ──
        if not st["sold_tp1"] and orig_sl>0 and current<=orig_sl:
            r = api_close_position(norm)
            if "error" not in r:
...
def auto_scan_and_trade(market="us"):
    """مسح + تداول تلقائي — يشتغل كلما في slots مفتوحة"""
    cfg = load_cfg()
    if not cfg.get("enabled"): return 0
    top = run_scan(market)
    if not cfg.get("auto_buy"): return 0
    bought = 0
    for s in top:
        if s["verdict"] != "BUY": continue
        if s["confidence"] < int(cfg.get("min_confidence",70)): continue
        res = safe_buy(s, "auto")
        if res["ok"]:
            bought += 1
        elif "وصلت الحد" in res.get("msg",""):
            break  # ما في slots — وقّف
    return bought

def scheduler_loop():
    """
    حلقة المجدول — تشتغل كل دقيقة:
    1. مراقبة المراكز (بيع ذكي) — فقط لو في مراكز
    2. إلغاء الأوامر المعلّقة: القديمة + المعلّقة أكثر من 4 ساعات
    3. السوق الأمريكي: مسح كل ساعة لو في slots فاضية
    4. الكريبتو 24/7: مسح كل ساعة
    """
    last_us_scan_hour = None
    ...
    while True:
        try:
            ...
            if cfg.get("enabled") and cfg.get("key"):
                ...
                # 1. مراقبة المراكز — فقط لو في مراكز فعلاً
                open_pos = api_positions()
                if open_pos:
                    monitor_positions()

                # 2. إلغاء الأوامر المعلّقة
                verify_pending_orders()

                cur_hour = datetime.utcnow().strftime("%Y-%m-%d-%H")

                # 3. السوق الأمريكي — كل ساعة لو في slots
                if "us" in cfg.get("markets",["us"]):
                    clock = api_clock()
                    if "error" not in clock and clock.get("is_open"):
                        max_open = int(cfg.get("max_daily_trades", 5))
                        open_count = len(open_pos)
                        has_slots = open_count < max_open
                        if last_us_scan_hour != cur_hour and has_slots:
                            last_us_scan_hour = cur_hour
                            res = auto_scan_and_trade("us")
                            print(f"✅ مسح أمريكي {cur_hour} — مراكز {open_count}/{max_open} — {res} صفقة جديدة")
    ...
    <div class="sub-body">
      ...
      <div id="sub-account">
        ...
        <div class="stat-strip" id="stat-strip">
          ...
          🎯 نجاح <b id="s-wr">0%</b> · 💰 <b id="s-pnl">$0</b> · 📁 مفتوحة <b id="s-open">0</b>/<b id="s-max">5</b>
        </div>
      ...
      <div id="sub-settings" class="hidden">
        <div class="grid-2">
          ...
          <div class="field"><label>حجم المركز الأقصى ($)</label><input class="input" id="c-max" type="number" value="2000"></div>
          <div class="field"><label>% من رأس المال لكل مركز</label><input class="input" id="c-pospct" type="number" value="2" step="0.5"></div>
          <div class="field"><label>حد الخسارة اليومي (%)</label><input class="input" id="c-loss" type="number" value="3" step="0.5"></div>
          <div class="field"><label>أقصى مراكز مفتوحة</label><input class="input" id="c-maxtrades" type="number" value="5"></div>
        ...
        <div class="strat-box">
          <div class="strat-h">📋 منطق التداول الآلي</div>
          • أقصى مراكز مفتوحة في نفس الوقت: <b>5</b><br>
          • حجم كل مركز: <b>2% من رأس المال</b> (أو الحد الأقصى لو أصغر)<br>
          • لو انباع مركز → البوت يفتح بديله في أول فرصة<br>
          • ربح <b>+2%</b> ← بيع <b>40%</b> · ربح <b>+4%</b> ← بيع <b>30%</b><br>
          • الباقي <b>30%</b> ← وقف متحرك <b>1.5%</b> تحت القمة<br>
          • وقف الخسارة الأصلي + حد خسارة يومي ← إغلاق الكل 🛡
        </div>
      </div>
...
function loadStats() {
  fetch('/api/stats').then(function(r){return r.json();}).then(function(d){
    ...
    document.getElementById('s-max').textContent = d.max_open||5;
  }).catch(function(){});
}
...
function loadConfig() {
  fetch('/api/config').then(function(r){return r.json();}).then(function(d){
    ...
    document.getElementById('c-max').value = d.max_position_usd||2000;
    document.getElementById('c-pospct').value = d.position_pct||2;
...
function saveConfig() {
  var cfg = {
    ...
    max_position_usd: +document.getElementById('c-max').value||2000,
    position_pct: +document.getElementById('c-pospct').value||2,
...
function loadLog() {
  fetch('/api/trades').then(function(r){return r.json();}).then(function(trades){
    ...
    if(!trades.length){ el.innerHTML='<div class="empty"><div class="empty-ico">📜</div>لا توجد صفقات بعد</div>'; return; }
    el.innerHTML = trades.slice().reverse().map(function(t){
      var cls = t.side==='buy'?'buy':(t.side==='sell'?'sell':'cancel');
      var ico = t.side==='buy'?'🟢':(t.side==='sell'?'🔴':'⏹');
      var who = t.source==='auto'?'🤖':'👤';
      var extra = t.reason ? (' · '+t.reason) : '';
      return '<div class="log-item '+cls+'"><span>'+ico+' '+t.symbol+' ×'+t.qty+extra+'</span>'+
        '<span>'+who+' '+t.time+'</span></div>';
    }).join('');
  });
}
...
function buyFromCard(s) {
  // نجيب الإعدادات الحالية لحساب الحجم الصح
  fetch('/api/config').then(function(r){return r.json();}).then(function(cfg){
    var equity = 0;
    fetch('/api/account').then(function(r){return r.json();}).then(function(acc){
      equity = acc.ok ? +acc.equity : 0;
      var pct = cfg.position_pct||2;
      var maxUsd = cfg.max_position_usd||2000;
      var sizeUsd = equity>0 ? Math.min(maxUsd, equity*pct/100) : maxUsd;
      var qty = Math.max(1, Math.floor(sizeUsd/s.price));
      if(!confirm('⚡ شراء '+s.name+' ('+s.code+')\nالسعر: $'+s.price+'\nحجم المركز: $'+sizeUsd.toFixed(0)+'\nالكمية: ~'+qty+'\nتأكيد؟')) return;
      fetch('/api/buy_signal',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(s)})
        .then(function(r){return r.json();}).then(function(d){
          if(d.ok){ toast('✅ تم الشراء: '+s.code+' ×'+d.qty+' ($'+Math.round(d.size_usd||sizeUsd)+')'); loadStats(); }
          else toast('❌ '+(d.msg||'فشل'));
        });
    });
  });
...
def stats():
    ...
    # ربح = pnl موجود وأكبر من صفر
    wins   = [t for t in sells if _safe_float(t.get("pnl",None)) > 0]
    losses = [t for t in sells if _safe_float(t.get("pnl",None)) <= 0]
    total_pnl = sum(_safe_float(t.get("pnl",0)) for t in sells)
    ...
    cfg = load_cfg()
    max_open = int(cfg.get("max_daily_trades",5))
    today_auto_buys = [t for t in buys if t.get("time","").startswith(today) and t.get("source")=="auto"]
    return jsonify({
        "total":len(buys),"closed":len(sells),"wins":len(wins),"losses":len(losses),
        "win_rate":wr,"total_pnl":round(total_pnl,2),
        "today":len(today_auto_buys),"max_daily":max_open,
        "open":open_count,"max_open":max_open,
    })
