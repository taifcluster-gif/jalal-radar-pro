...
def analyze_symbol(code, name, is_crypto=False, live_price=None):
    ...
    try:
        ...
        # ── فلتر السعر: $5–$150 فقط (مناسب لرأس مال صغير) ──
        if not is_crypto and (price < 5 or price > 150):
            return None
...
def auto_scan_and_trade(market="us"):
    ...
    for s in top:
        ...
        # ── Volume spike: حجم التداول لازم يكون أعلى من المعدل بـ 20% على الأقل ──
        if not s.get("is_crypto") and s.get("vr", 0) < 1.2:
