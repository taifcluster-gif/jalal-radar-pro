...
import warnings, threading, os, json, time, csv, io, traceback
import urllib.request, urllib.parse
import sys
import risk_manager as rm  # v3.3 — وحدة إدارة المخاطر (الحجم الذكي + سقوف التعرض + حد القطاع)
try:
    from backtest_engine import backtest_symbol, summarize  # v3.4 — محرك الاختبار التاريخي
except ImportError:
    backtest_symbol = summarize = None
try:
    import websocket  # للاختبار اللحظي (v3.4) — pip install websocket-client
except ImportError:
    websocket = None
warnings.filterwarnings("ignore")

# ── إصلاح اختفاء اللوجات على Render: stdout بدون تخزين مؤقت ──
try:
...
def sahmk_get(path):
    ...
    try:
        req = urllib.request.Request(SAHMK_BASE+path, headers={"Authorization":f"Bearer {key}","Accept":"application/json"})
        with urllib.request.urlopen(req, timeout=10) as r:
            return json.loads(r.read())
    except Exception as e: return {"error": str(e)}

def sahmk_price(symbol):
    r = sahmk_get(f"/quote/{symbol}/")
    try: return float(r.get("price") or r.get("last_price") or r.get("close") or 0)
    except: return None

def sahmk_history(symbol):
    r = sahmk_get(f"/historical/{symbol}/")
    try:
...
SA_STOCKS = {
    "2222":"أرامكو السعودية","1180":"الأهلي السعودي","1120":"الراجحي",
    "2010":"سابك","7010":"stc","4200":"الاتصالات السعودية",
    "1211":"معادن","1030":"الرياض","2380":"بترو رابغ",
    "2350":"المتطورة","3010":"نمي","1010":"الرياض البنك",
    "2290":"أبو قير","4001":"تحلية","2250":"جازان",
    "6010":"سدافكو","3020":"الإنماء الكيماوي","4030":"مكة للإنشاء",
    "8010":"الدريس","2020":"سابك للبتروكيماويات",
}
...
def load_cfg():
    ...
    defaults = {
        ...
        "max_daily_trades": rm.MAX_TRADES_PER_DAY,  # (v3.3) الافتراضي من وحدة المخاطر = 8
        "daily_loss_pct": 3.0,
        "min_confidence": 70,
        "auto_buy": True,
        "auto_sell": True,
        "markets": ["us"],          # us, crypto, sa
        "sahmk_key": os.environ.get("SAHMK_KEY",""),
        "buy_order_type": "market", # market = تنفيذ مضمون · limit = سعر محدد
        "tp1_pct": 3.0,             # (v3.4) كان 2.0 — وسّعناه لأن الاختبار التاريخي أثبت الخسارة أكبر من الربح
    ...
    if os.path.exists(ALPACA_CONFIG_FILE):
        try:
            ...
            # (v3.3) ترقية لمرة واحدة: القيمة القديمة 5 المحفوظة ترتفع للحد الجديد
            if not merged.get("risk_v33_migrated") and int(merged.get("max_daily_trades", 5)) < rm.MAX_TRADES_PER_DAY:
                merged["max_daily_trades"] = rm.MAX_TRADES_PER_DAY
                merged["risk_v33_migrated"] = True
                try: save_cfg(merged)
                except: pass
            return merged
        except: pass
...
def _req(base, path, method="GET", data=None, timeout=6):
    cfg = load_cfg()
...
# ── تاريخ الصفقات من Alpaca (لا يضيع عند deploy) ──
def api_activities(limit=100):
    """
...
def alpaca_history(ticker, is_crypto=False, days=730):
    """بيانات تاريخية يومية من Alpaca مباشرة — لا تُحظر لأننا عملاء عندهم"""
    cfg = load_cfg()
    if not cfg.get("key"):
        log(f"🔴 [ALPACA-ERR] {ticker}: لا يوجد مفتاح API في الإعدادات (ALPACA_KEY)")
        return pd.DataFrame()
    try:
        start = (datetime.now(timezone.utc) - pd.Timedelta(days=days)).strftime("%Y-%m-%d")
        if is_crypto:
            sym = ticker.replace("-USD", "/USD") if "-USD" in ticker else ticker
            path = (f"/v1beta3/crypto/us/bars?symbols={urllib.parse.quote(sym)}"
                    f"&timeframe=1Day&start={start}&limit=1000")
            r = _req(cfg["data_url"], path, timeout=15)
            if isinstance(r, dict) and r.get("error"):
                log(f"🔴 [ALPACA-ERR] {ticker}: رد Alpaca بخطأ: {r['error']} | data_url={cfg['data_url']}")
                return pd.DataFrame()
            bars = r.get("bars", {}).get(sym, []) if isinstance(r, dict) else []
        else:
            # feed=iex مجاني ويكفي للتحليل اليومي (sip يتطلب اشتراك مدفوع)
            path = (f"/v2/stocks/{ticker}/bars?timeframe=1Day&start={start}"
                    f"&limit=1000&adjustment=split&feed=iex")
            r = _req(cfg["data_url"], path, timeout=15)
            if isinstance(r, dict) and r.get("error"):
                log(f"🔴 [ALPACA-ERR] {ticker}: رد Alpaca بخطأ: {r['error']} | data_url={cfg['data_url']}")
                return pd.DataFrame()
            bars = r.get("bars", []) if isinstance(r, dict) else []
        if not bars:
            log(f"🟡 [ALPACA-WARN] {ticker}: الرد سليم لكن بدون شموع (bars فارغة)")
            return pd.DataFrame()
        ...
        log(f"🔴 [ALPACA-ERR] {ticker}: استثناء أثناء الجلب/التحويل: {type(e).__name__}: {e}")
        return pd.DataFrame()

def get_history(ticker, period="2y", interval="1d", retries=2):
    """بيانات تاريخية: Alpaca أولاً (موثوق على Render)، ثم yfinance كاحتياطي"""
    is_crypto = "-USD" in ticker
    # 1) Alpaca — المصدر الأساسي (75% من طلبات yfinance كانت تُحظر على Render)
    df = alpaca_history(ticker, is_crypto)
    if not df.empty and len(df) >= 50:
        return df
    # 2) yfinance — احتياطي فقط (للتاسي .SR أو لو Alpaca ما رجّعت شي)
...
def analyze_symbol(code, name, is_crypto=False, live_price=None):
    ...
    try:
        ...
        price_tier = "normal"
        if not is_crypto:
            # ✅ إصلاح: حذف سقف الـ 150$ — كان يستبعد أغلب القائمة (AAPL, MSFT, NVDA...)
            # ويُحسب خطأً كـ"فشل جلب بيانات". الكسور (fractional) تسمح بأي سعر بأمان.
            if price >= 5:
                price_tier = "normal"
            elif price >= 1:
                price_tier = "low"
        ...
        # ── (v3.3) الحجم النسبي الذكي: تصحيح شمعة اليوم الجزئية حسب الوقت المنقضي من الجلسة ──
        # المشكلة القديمة: حجم اليوم (الجزئي) ÷ متوسط 20 يوم كامل = أرقام مظلومة صباحاً (0.2x-0.4x)
        # التصحيح يُطبّق فقط إذا: سهم أمريكي + آخر شمعة هي شمعة اليوم + الجلسة مفتوحة فعلياً
        raw_vr = float(c["Volume"].iloc[last]) / float(va.iloc[last]) if float(va.iloc[last]) > 0 else 1.0
        smart_vr = raw_vr
        if not is_crypto:
            try:
                _last_bar_today = pd.Timestamp(c.index[last]).date() == datetime.now(timezone.utc).date()
            except Exception:
                _last_bar_today = False
            _elapsed = rm.session_elapsed_fraction()
            if _last_bar_today and 0.05 < _elapsed < 1.0:
                smart_vr = raw_vr / max(rm.expected_volume_fraction(_elapsed), 0.05)
        ...
        c10 = smart_vr > 1.2  # (v3.3) حجم ذكي مصحّح بالوقت بدل الشمعة الجزئية
        ...
        sl = round(price - atr_v*1.0, 2)  # (v3.4) كان 1.5×ATR — ضيّقناه، الاختبار التاريخي أثبت الخسارة تفوت الربح
        ...
        vr = round(smart_vr, 1)  # (v3.3) الحجم الذكي
        ...
        vr_now = round(smart_vr, 1)  # (v3.3) الحجم الذكي — يصلح تصنيف Tier 2/3 صباحاً
        entry_tier = None
        if verdict == "BUY":
            entry_tier = 1
            ...
            # ✅ إصلاح: BUY_COND بدون حجم مرتفع = Tier 3 (0.5%) بدل tier=None
            # قبل الإصلاح كانت كل إشارات الشراء المشروط تُتجاهل بالكامل
            entry_tier = 3
...
_pending_logged = set()  # (v3.3) لمنع تكرار رسالة الأمر المعلق
...
def get_today_auto_buys_from_alpaca():
    """
    (v3.4) عدّاد صفقات اليوم من سجل Alpaca الدائم — بديل عن load_trades()
    المحلي اللي ينمسح مع كل إعادة نشر (Render قرص غير دائم)، وكان هذا يعطّل
    قافل الأمان اليومي بصمت بعد أي رفعة كود. نرجع قائمة رموز اشتُريت اليوم فقط.
    ترجع None لو فشل الاتصال بـAlpaca — عشان المتصل يوقف الشراء احتياطاً
    (فشل آمن: نمنع الشراء عند الشك، مو نسمح به).

    (v3.5) إصلاح: صار يضيف رموز الأوامر المفتوحة/المعلّقة اليوم أيضاً، مو بس
    الأنشطة المنفذة (activities). قبل كذا، أمر "accepted" ما انفّذ بعد كان
    يختفي من هذا الفحص تماماً فيسمح بإعادة إرسال نفس الرمز بلا حد.
    """
    try:
        acts = api_activities(200)
        today = datetime.now().strftime("%Y-%m-%d")
        symbols = []
        for a in acts:
            t = a.get("transaction_time","")
            if t.startswith(today) and a.get("side")=="buy":
                symbols.append(a.get("symbol",""))

        # (v3.5) أضف رموز الأوامر المفتوحة اليوم (accepted/new/pending_new) عشان
        # ما نكرر نفس الرمز وهو أصلاً معلّق بانتظار التنفيذ
        try:
            open_orders = api_open_orders()
            for o in open_orders:
                created = o.get("created_at","")[:10]
                if created == today and o.get("side")=="buy":
                    sym = o.get("symbol","").replace("/USD","-USD")
                    symbols.append(sym)
        except Exception:
            pass
...
scan_state = {
    ...
    "sa":     {"data": [], "last": None, "status": "idle", "progress": 0, "scanned": 0},
}
...
def run_scan(market="us"):
    ...
    log(f"📊 [SCAN {market}] فُحص {total} | نجح التحليل {len(results)} | فشل جلب البيانات {failed} | إشارات شراء {len(buys)} | أفضل النتائج {[s['code'] for s in top[:3]]}")
    if total > 0 and failed / total > 0.7:
        log(f"🚨 [SCAN {market}] تحذير: {failed}/{total} استُبعدوا — ابحث في السجل عن ALPACA-ERR/ALPACA-WARN لمعرفة إن كان السبب فشل بيانات فعلي")
    return top

# ════════════════════════════════════════════════════════════
# مسح وتحليل تاسي
# ════════════════════════════════════════════════════════════
def analyze_symbol_sa(df, code, name, price):
    """تحليل JRF للأسهم السعودية"""
    try:
        c = df.copy()
        ...
        def _macd(s):
            ml=s.ewm(12,adjust=False).mean()-s.ewm(26,adjust=False).mean()
            sg=ml.ewm(9,adjust=False).mean(); return ml,sg
        e20=_ema(c["Close"],20); e50=_ema(c["Close"],50); e200=_ema(c["Close"],200)
        rv=_rsi(c["Close"]); ml,sg=_macd(c["Close"])
        rsi=float(rv.iloc[-1]); macd_val=float(ml.iloc[-1]); macd_sig=float(sg.iloc[-1])
        ema20=float(e20.iloc[-1]); ema50=float(e50.iloc[-1]); ema200=float(e200.iloc[-1])
        vol_now=float(c["Volume"].iloc[-1]); vol_avg=float(c["Volume"].rolling(20).mean().iloc[-1]) or 1
        vr=round(vol_now/vol_avg,2)
        score=0
        if price>ema20: score+=3
        if price>ema50: score+=3
        if price>ema200: score+=2
        if 40<rsi<70: score+=2
        if macd_val>macd_sig: score+=3
        if vr>1.2: score+=2
        if price>float(c["Close"].iloc[-2]): score+=2
        if rsi>50: score+=1
        if macd_val>0: score+=2
        if score>=15: verdict="BUY"
        elif score>=11: verdict="BUY_COND"
        elif score>=7: verdict="WATCH"
        else: verdict="AVOID"
        confidence=min(99,int(score/20*100))
        atr=float(c["High"].sub(c["Low"]).rolling(14).mean().iloc[-1])
        t1=round(price+atr*1.5,2); t2=round(price+atr*3.0,2); sl=round(price-atr*1.0,2)
        rr=round((t1-price)/(price-sl),2) if price>sl else 0
        ptp=round((t1-price)/price*100,1) if price>0 else 0
        psl=round((price-sl)/price*100,1) if price>0 else 0
        # ADX
        try:
            hi=c["High"]; lo=c["Low"]; cl=c["Close"]
            tr=pd.concat([hi-lo,(hi-cl.shift()).abs(),(lo-cl.shift()).abs()],axis=1).max(axis=1)
            atr14=tr.rolling(14).mean()
            dm_p=(hi.diff().clip(lower=0)); dm_n=(-lo.diff().clip(upper=0))
        ...
        e_tier=1 if score>=15 else (2 if score>=11 else 3)
        return {
            "code":code,"name":name,"price":round(price,2),"currency":"SAR",
            "score":score,"confidence":confidence,"verdict":verdict,"rr":rr,
            "t1":t1,"t2":t2,"sl":sl,"lb":round(price*0.99,2),
            "ptp":ptp,"psl":psl,"rsi":round(rsi,1),"adx":adx_val,
            "trend":trend,"eta":eta,"vr":vr,"entry_tier":e_tier,
            "is_crypto":False,"is_sa":True,"live":False,
        }
    except: return None

def run_scan_sa():
    """مسح تاسي — البيانات من سهمك أو yfinance، التحليل JRF"""
    market="sa"
    scan_state[market]["status"]="scanning"; scan_state[market]["progress"]=0
    items=list(SA_STOCKS.items()); total=len(items)
    results=[]; lock=threading.Lock(); done=[0]

    def scan_one_sa(code, name):
        try:
    ...
    with ThreadPoolExecutor(max_workers=2) as ex:
        futs=[ex.submit(scan_one_sa,c,n) for c,n in items]
        time.sleep(0.3)
        for f in as_completed(futs): pass
    ...
    if len(top)<5:
        ...
        top+=watch[:5-len(top)]
    medals=["🥇","🥈","🥉","4️⃣","5️⃣"]
    for i,s in enumerate(top): s["medal"]=medals[i] if i<len(medals) else str(i+1)
    fav_set=set(load_favs().get(market,[]))
    for s in top: s["is_fav"]=s["code"] in fav_set
    scan_state[market]["data"]=top; scan_state[market]["last"]=datetime.now().strftime("%Y-%m-%d %H:%M")
    scan_state[market]["scanned"]=len(results); scan_state[market]["status"]="done"
    scan_state[market]["progress"]=100
    return top
...
def safe_buy(signal, source="auto"):
    ...
    today = datetime.now().strftime("%Y-%m-%d")
    today_buy_symbols = get_today_auto_buys_from_alpaca()
    if today_buy_symbols is None:
        return {"ok": False, "msg": "تعذّر التحقق من حد اليوم من Alpaca — نوقف الشراء احتياطاً"}

    # 2. اشترينا نفس السهم اليوم؟
    if code in today_buy_symbols:
        return {"ok": False, "msg": "اشترينا هذا السهم اليوم"}

    # 3. حد الصفقات اليومي
    if source=="auto" and len(today_buy_symbols) >= int(cfg.get("max_daily_trades", rm.MAX_TRADES_PER_DAY)):
        return {"ok": False, "msg": f"وصلنا حد الصفقات اليومي ({int(cfg.get('max_daily_trades', rm.MAX_TRADES_PER_DAY))})"}

    # ── (v3.3) حد القطاع: أقصى صفقتين جديدتين لكل قطاع في اليوم (تلقائي فقط) ──
    if source == "auto" and not is_crypto:
        _sector = rm.get_sector(code)
        _sector_count = sum(1 for s in today_buy_symbols if rm.get_sector(s) == _sector)
        if _sector_count >= rm.MAX_TRADES_PER_SECTOR:
            return {"ok": False, "msg": f"وصلنا حد قطاع {_sector} اليوم ({rm.MAX_TRADES_PER_SECTOR} صفقة)"}
    ...
    # (v3.4) تعطيل Tier 1 مؤقتاً — الاختبار التاريخي (5448 صفقة/سنتين) أثبت إنه
    ...
    # ── (v3.3) سقوف التعرض: للسهم الواحد وللمحفظة كاملة ──
    if not is_crypto and equity and equity > 0:
        try:
            _pos_dict = rm.positions_to_dict(api_positions())
        except Exception:
            _pos_dict = {}

        # سقف السهم الواحد (شامل أي مركز مفتوح فيه)
        _cur_sym = float(_pos_dict.get(code.upper(), 0.0))
        _max_sym = equity * rm.MAX_SYMBOL_EXPOSURE_PCT
        if _cur_sym + max_usd > _max_sym:
            return {"ok": False, "msg": (f"تجاوز سقف السهم الواحد "
                    f"({rm.MAX_SYMBOL_EXPOSURE_PCT*100:.0f}% = ${_max_sym:,.0f}، "
                    f"المفتوح حالياً ${_cur_sym:,.0f})")}

        # سقف التعرض الكلي للمحفظة
        _total_exp = sum(_pos_dict.values())
        _max_total = equity * rm.MAX_TOTAL_EXPOSURE_PCT
        if _total_exp + max_usd > _max_total:
            return {"ok": False, "msg": (f"تجاوز سقف التعرض الكلي "
                    f"({rm.MAX_TOTAL_EXPOSURE_PCT*100:.0f}% = ${_max_total:,.0f}، "
                    f"الحالي ${_total_exp:,.0f})")}
    ...
    # ⚠️ لكن فقط إذا الكمية >= 1 — التقريب من 0.5 إلى 1 سهم يضاعف حجم الصفقة والمخاطرة!
    if "error" in result and not is_crypto and qty != int(qty):
        if qty < 1:
            log(f"❌ {code}: رفض كسري وحجم الصفقة أقل من سهم كامل ({qty}) — إلغاء لحماية حجم المخاطرة")
            return {"ok": False, "msg": f"الوسيط يرفض الكسور و{qty} سهم أقل من سهم كامل — أُلغيت الصفقة"}
        int_qty = int(qty)  # نقرّب للأسفل دائماً (مخاطرة أقل، ليس أكثر)
...
def verify_pending_orders():
    ...
    - (v3.5) لو أمر معلّق من نفس اليوم لكن عدّى 30 دقيقة بدون تنفيذ → يلغيه
      (قبل كذا كان يضل عالق طول اليوم بدون ما ينحذف، ويمنع أي رادع يشتغل عليه)
    """
    cfg = load_cfg()
    ...
    for o in open_orders:
        ...
        if created_date and created_date < today:
            ...
            add_trade({
                ...
                "time":now.strftime("%Y-%m-%d %H:%M"), "source":"auto",
            })
            continue

        # (v3.5) أمر من نفس اليوم لكن عالق أكثر من 30 دقيقة
        if created_date == today and created_str:
            try:
                ...
                if age_min > 30:
                    ...
                    add_trade({
                        "symbol":o.get("symbol",""), "side":"cancel",
                        "qty":o.get("qty","0"),
                        "reason":f"أمر عالق {round(age_min)} دقيقة بدون تنفيذ — أُلغي",
                        "time":now.strftime("%Y-%m-%d %H:%M"), "source":"auto",
                    })
            except Exception:
                pass

_exit_check_lock = threading.Lock()  # يمنع تعارض بين الفحص الدوري والفحص اللحظي على نفس المركز
_last_realtime_check = {}            # {symbol: timestamp} — تحكم بالسرعة عشان ما نغرق Alpaca بطلبات

def _evaluate_exit_for_position(pos, cfg, trades, pstate, tp1_pct, tp2_pct, trail_pct,
                                  symbols_with_pending_sell, price_override=None):
    """
    منطق الخروج لمركز واحد (TP1/TP2/Trailing/وقف الخسارة/حماية الشورت).
    مستخرج من monitor_positions() عشان نقدر نستدعيه من مصدرين:
    1) الفحص الدوري كل 15 دقيقة (price_override=None → يستخدم سعر Alpaca المخزّن بالمركز)
    2) المراقبة اللحظية (price_override=السعر اللحظي اللي وصل توّه عبر WebSocket)
    ترجع True لو صار بيع (جزئي أو كامل)، عشان المتصل يعرف إذا احتاج يحدّث حالته.
    """
    symbol = pos.get("symbol","")
    norm = symbol.replace("/USD","-USD") if "/USD" in symbol else symbol

    if norm in symbols_with_pending_sell:
        return False

    try:
        current = float(price_override) if price_override is not None else float(pos.get("current_price",0))
        entry = float(pos.get("avg_entry_price",0))
        qty = float(pos.get("qty",0))
    except Exception:
        return False
    if current<=0 or entry<=0: return False

    if qty < 0:
        api_close_position(norm)
        add_trade({"symbol":norm,"side":"buy","qty":abs(qty),
                  "reason":"⚠️ إغلاق طارئ - شورت غير مدعوم (الاستراتيجية للشراء فقط)",
                  "exit_price":current,"pnl":round((entry-current)*abs(qty),2),
                  "time":datetime.now().strftime("%Y-%m-%d %H:%M"),"source":"auto"})
        pstate.pop(norm,None)
        return True
    ...
    if pos_tier == 3:
        orig_sl = max(orig_sl, round(entry * 0.985, 2))
        pos_trail_pct = min(trail_pct, 1.0)

    current_sl = st.get("adj_sl", orig_sl)
    if not st["sold_tp1"] and pnl_pct >= 1.5:
        current_sl = max(current_sl, entry)
        st["adj_sl"] = current_sl

    sold = False
    source_tag = "auto-realtime" if price_override is not None else "auto"
    ...
    if not st["sold_tp1"] and pnl_pct >= tp1_pct:
        ...
        if "error" not in r:
            ...
            add_trade({"symbol":norm,"side":"sell","qty":q,"reason":f"هدف 1 (+{tp1_pct}%) بيع 40%",
                      ...
                      "time":datetime.now().strftime("%Y-%m-%d %H:%M"),"source":source_tag})
...
def monitor_positions():
    ...
    # ── الأوامر المفتوحة: نتجاهل أي سهم عنده أمر بيع معلّق ──
    open_orders = api_open_orders()
    symbols_with_pending_sell = set()
    for o in open_orders:
        if o.get("side") == "sell":
            sym = o.get("symbol","").replace("/USD","-USD")
            symbols_with_pending_sell.add(sym)
    ...
    with _exit_check_lock:
        for pos in positions:
            symbol = pos.get("symbol","")
            norm = symbol.replace("/USD","-USD") if "/USD" in symbol else symbol
            if norm in symbols_with_pending_sell:
                _pk = datetime.now().strftime("%Y-%m-%d") + ":" + norm
                if _pk not in _pending_logged:
                    _pending_logged.add(_pk)
                    if len(_pending_logged) > 200: _pending_logged.clear()
                    print(f"⏭️ {norm} — عنده أمر بيع معلّق، نتجاهله")
                continue
            _evaluate_exit_for_position(pos, cfg, trades, pstate, tp1_pct, tp2_pct, trail_pct,
                                         symbols_with_pending_sell)
        save_posstate(pstate)


def realtime_on_tick(symbol, price):
    """
    (v3.4) يُستدعى من RealtimeExitMonitor عند كل صفقة لحظية وصلت عبر WebSocket.
    يطبّق نفس منطق الخروج فوراً بدل انتظار دورة الـ15 دقيقة — بس فقط للرمز
    اللي وصلت له صفقة، وبسقف سرعة (throttle) عشان ما نطلب من Alpaca أكثر من
    مرة كل ثانيتين لنفس الرمز.
    """
    now = time.time()
    last = _last_realtime_check.get(symbol, 0)
    if now - last < 2.0:
        return  # throttle — تجاهل التيكات المتقاربة جداً لنفس الرمز
    _last_realtime_check[symbol] = now

    try:
        cfg = load_cfg()
        if not cfg.get("enabled") or not cfg.get("auto_sell"):
            return

        positions = api_positions()
        pos = next((p for p in positions
                    if p.get("symbol","").replace("/USD","-USD") == symbol), None)
        if not pos:
            return  # ما عندنا مركز بهذا الرمز — تجاهل

        open_orders = api_open_orders()
        symbols_with_pending_sell = {
            o.get("symbol","").replace("/USD","-USD")
            for o in open_orders if o.get("side")=="sell"
        }
        if symbol in symbols_with_pending_sell:
            return
        ...
        with _exit_check_lock:
            changed = _evaluate_exit_for_position(pos, cfg, trades, pstate, tp1_pct, tp2_pct,
                                                   trail_pct, symbols_with_pending_sell,
                                                   price_override=price)
            if changed:
                save_posstate(pstate)
                print(f"⚡ [لحظي] تدخّل فوري على {symbol} @ ${price}")
    except Exception:
        traceback.print_exc()
...
def auto_scan_and_trade(market="us"):
    ...
    for s in top:
        ...
        if e_tier not in (1, 2, 3):
            log(f"⏭️ {s['code']} تجاهل — غير مصنّف للشراء (tier={e_tier}, verdict={s.get('verdict')})")
            continue
        # ✅ إصلاح: البوابات القديمة (75/80) كانت مستحيلة رياضياً —
        # معادلة الثقة لإشارة BUY_COND سقفها ~70% (score<15 يعني 42 نقطة كحد أقصى من 60)
        # المخاطرة أصلاً مضبوطة بحجم المركز (Tier2=1.2%, Tier3=0.5%)، فالبوابة أخف
        min_conf = int(cfg.get("min_confidence", 70))
        if e_tier == 2: min_conf = 62
        if e_tier == 3: min_conf = 58
        if s["confidence"] < min_conf:
            log(f"⏭️ {s['code']} تجاهل — ثقة {s['confidence']}% أقل من المطلوب {min_conf}% (Tier {e_tier})")
            continue
        # ── Volume spike: حجم التداول لازم يكون أعلى من المعدل بـ 20% على الأقل (المستوى 1 فقط، لأن 2و3 محسوبين أصلاً بشرط حجم أعلى) ──
        # ✅ إصلاح: 1.2x كانت ترفض إشارات BUY قوية بحجم طبيعي (XOM 0.9x-1.1x)
        if e_tier == 1 and not s.get("is_crypto") and s.get("vr", 0) < 1.0:
            log(f"⏭️ {s['code']} تجاهل — حجم منخفض (ذكي {s.get('vr',0)}x)")
            continue
        res = safe_buy(s, "auto")
        if res["ok"]:
            bought += 1
            log(f"💰 شراء تلقائي: {s['code']} (Tier {e_tier}, ثقة {s['confidence']}%)")
        else:
            log(f"❌ رفض شراء {s['code']}: {res.get('msg','')}")
    return bought
...
def scheduler_loop():
    ...
    while True:
        try:
            ...
            # 🫀 نبضة كل 15 دقيقة — توضح حالة النظام كاملة في اللوجات
            if time.time() - last_heartbeat > 900:
                last_heartbeat = time.time()
                clk = cached_clock() if cfg.get("key") else {"error": "no key"}
                mkt = "مفتوح ✅" if clk.get("is_open") else ("خطأ اتصال ⚠️" if "error" in clk else "مغلق 🔒")
                log(f"🫀 [HEARTBEAT] مفعّل={cfg.get('enabled')} | مفتاح={'موجود' if cfg.get('key') else 'مفقود ❌'} | "
                    f"شراء تلقائي={cfg.get('auto_buy')} | السوق الأمريكي: {mkt} | الأسواق: {cfg.get('markets')}")
                ...
                if "us" in cfg.get("markets",["us"]):
                    ...
                    if is_open:
                        ...
                        if last_us_scan_day != cur_15:
                            log(f"🔍 بدء مسح تلقائي (نافذة {cur_15})...")
                            res = auto_scan_and_trade("us")
                            last_us_scan_day = cur_15
                            log(f"✅ مسح أمريكي تلقائي {cur_15} — {res or 0} صفقة")
...
<div class="shell">
  <div class="mkt-tabs">
    <button class="mkt-tab" id="mt-sa" onclick="setMarket('sa')">🇸🇦 تاسي</button>
  ...
  <div class="panel">
    ...
    <div class="sub-body">
      ...
      <div id="sub-positions" class="hidden">
        ...
        <a href="/api/export_excel" class="btn btn-ghost" style="margin-bottom:12px;margin-right:8px;text-decoration:none;display:inline-block;">📥 تصدير Excel (CSV)</a>
      ...
      <div id="sub-settings" class="hidden">
        <div class="grid-2">
          ...
          <div class="field"><label>أقصى صفقات يومياً</label><input class="input" id="c-maxtrades" type="number" value="8"></div>
        ...
        <div class="field" style="margin-top:11px;"><label>🇸🇦 مفتاح سهمك API</label><input class="input" id="c-sahmk" type="password" placeholder="shmk_test_…"></div>
        ...
        <button class="btn btn-ghost btn-block" style="margin-top:8px;" onclick="testRealtime()">⚡ اختبار الاتصال اللحظي (تجريبي)</button>
        <div id="rt-test-log" style="margin-top:10px;font-size:13px;display:none;background:#f5f5f5;border-radius:8px;padding:10px;white-space:pre-wrap;direction:ltr;text-align:left;"></div>
        <div class="strat-box">
      ...
      <div id="sub-log" class="hidden">
        <button class="btn btn-ghost" onclick="loadLog()" style="margin-bottom:12px;">🔄 تحديث</button>
        <a href="/api/export_excel" class="btn btn-ghost" style="margin-bottom:12px;margin-right:8px;text-decoration:none;display:inline-block;">📥 تصدير Excel (CSV)</a>
        <a href="/api/export_internal_log" class="btn btn-ghost" style="margin-bottom:12px;margin-right:8px;text-decoration:none;display:inline-block;">📊 تصدير السجل الداخلي (بالأسباب)</a>
  ...
  <!-- (v3.4) الاختبار التاريخي -->
  <div class="scan-zone" style="margin-top:12px;">
    <button class="btn btn-ghost btn-block" onclick="runBacktest()">📈 اختبار تاريخي (سنتين — يأخذ عدة دقائق)</button>
    <div id="backtest-status" style="margin-top:10px;font-size:13px;display:none;background:#f5f5f5;border-radius:8px;padding:10px;white-space:pre-wrap;"></div>
  </div>
...
function setMarket(m) {
  market = m;
  document.getElementById('mt-sa').classList.toggle('on', m==='sa');
  document.getElementById('mt-us').classList.toggle('on', m==='us');
  document.getElementById('mt-crypto').classList.toggle('on', m==='crypto');
  var lbls={'us':'🔍 مسح السوق الأمريكي','crypto':'🔍 مسح الكريبتو','sa':'🔍 مسح تاسي 🇸🇦'};
  document.getElementById('scan-btn').innerHTML = lbls[m]||'🔍 مسح';
  loadResults();
  loadFavorites();
}
...
function loadAccount() {
  ...
  fetch('/api/account').then(function(r){
    ...
    if (d.ok) {
      ...
      toast('⚠️ فشل الاتصال: ' + (d.msg || 'سبب غير معروف'));
    }
  }).catch(function(err){
    toast('⚠️ فشل تحديث بيانات الحساب (' + err.message + ')');
  });
}
...
function loadStats() {
  fetch('/api/stats').then(function(r){
    ...
    toast('⚠️ فشل تحديث الإحصائيات ('+err.message+')');
  });
}
...
function loadConfig() {
  fetch('/api/config').then(function(r){return r.json();}).then(function(d){
    ...
    document.getElementById('c-maxtrades').value = d.max_daily_trades||8;
    ...
    if(document.getElementById('c-sahmk')) document.getElementById('c-sahmk').value = d.sahmk_key||'';
  }).catch(function(){
    // ── لا نبلع الخطأ بصمت — القيم المعروضة قد تكون افتراضية مو حقيقية ──
    toast('⚠️ ما قدرنا نجيب الإعدادات الحقيقية من السيرفر — القيم المعروضة قد لا تطابق المخزّن فعلياً');
  });
}
...
function saveConfig() {
  var cfg = {
    ...
    max_daily_trades: +document.getElementById('c-maxtrades').value||8,
    ...
    sahmk_key: document.getElementById('c-sahmk') ? document.getElementById('c-sahmk').value.trim() : ''
  };
...
function testRealtime() {
  var box = document.getElementById('rt-test-log');
  box.style.display = 'block';
  box.textContent = '⏳ جاري الاختبار (قد يأخذ لين 8 ثواني)...';
  fetch('/api/test-realtime').then(function(r){return r.json();}).then(function(d){
    box.textContent = (d.log||[]).join('\n');
    if(d.ok){ toast('✅ الاتصال اللحظي شغال'); } else { toast('⚠️ فيه مشكلة — شوف التفاصيل تحت'); }
  }).catch(function(err){
    box.textContent = '❌ فشل الاتصال بالسيرفر نفسه: ' + err.message;
    toast('❌ فشل الاختبار');
  });
}
...
function runBacktest() {
  var box = document.getElementById('backtest-status');
  box.style.display = 'block';
  box.textContent = '⏳ جاري بدء الاختبار...';
  fetch('/api/backtest/start', {method:'POST'}).then(function(r){return r.json();}).then(function(d){
    if(!d.ok){ box.textContent = '❌ ' + d.msg; return; }
    if(_btPoll) clearInterval(_btPoll);
    _btPoll = setInterval(pollBacktest, 3000);
  }).catch(function(err){
    box.textContent = '❌ فشل الاتصال: ' + err.message;
  });
}

function pollBacktest() {
  var box = document.getElementById('backtest-status');
  fetch('/api/backtest/status').then(function(r){return r.json();}).then(function(d){
    if(d.error){
      box.textContent = '❌ خطأ: ' + d.error;
      clearInterval(_btPoll); _btPoll = null;
      return;
    }
    if(d.running){
      box.textContent = '⏳ جاري الاختبار: ' + d.progress + ' / ' + d.total + ' سهم...';
      return;
    }
    clearInterval(_btPoll); _btPoll = null;
    if(!d.result){ box.textContent = 'ما طلعت نتيجة.'; return; }
    var r = d.result;
    var txt = '✅ النتيجة (سنتين، ' + r.total_trades + ' صفقة):\n';
    txt += 'نسبة النجاح: ' + r.win_rate + '%  (' + r.wins + ' ربح / ' + r.losses + ' خسارة)\n';
    txt += 'متوسط الربح: $' + r.avg_win + '  |  متوسط الخسارة: $' + r.avg_loss + '\n';
    txt += 'الصافي الكلي: $' + r.total_pnl + '\n\n';
    txt += 'حسب المستوى (Tier):\n';
    for (var tier in r.by_tier) {
      var t = r.by_tier[tier];
      txt += '  Tier ' + tier + ': ' + t.trades + ' صفقة، نجاح ' + t.win_rate + '%، صافي $' + t.total_pnl + '\n';
    }
    box.textContent = txt;
    toast('✅ خلص الاختبار التاريخي');
  }).catch(function(){});
}
  ...
  fetch('/api/positions').then(function(r){return r.json();}).then(function(d){
    ...
    el.innerHTML = d.positions.map(function(p){
      ...
      return '<div class="pos"><div><div class="pos-sym">'+p.symbol+'</div>'+
        '<div class="pos-info">'+p.qty+' · دخول $'+p.entry+' · الآن $'+p.current+'</div>'+
        '<div class="pos-info" style="opacity:.75;">💵 المستثمر $'+p.invested+' · القيمة الحالية $'+p.value+'</div></div>'+
...
function loadLog() {
  ...
  el.innerHTML = '<div class="empty"><div class="empty-ico">⏳</div>جاري التحميل...</div>';

  fetch('/api/alpaca_history').then(function(r){return r.json();}).then(function(acts){
    if(!acts||!acts.length){
      el.innerHTML='<div class="empty"><div class="empty-ico">📜</div>لا توجد صفقات مسجّلة في Alpaca</div>';
      if(sumEl) sumEl.innerHTML=''; return;
    }

    var buys  = acts.filter(function(a){return a.side==='buy';});
    var sells = acts.filter(function(a){return a.side==='sell';});
    var totalVol = acts.reduce(function(a,t){return a+(t.price*t.qty);},0);

    if(sumEl) sumEl.innerHTML = [
      ['📊 إجمالي', acts.length + ' نشاط'],
      ['🟢 شراء', buys.length],
      ['🔴 بيع', sells.length],
      ['💵 حجم', '$'+totalVol.toFixed(0)],
    ].map(function(s){
      return '<div style="background:var(--surface-2);border-radius:10px;padding:10px;text-align:center;">'
        ...
        +'<div style="font-weight:700;color:var(--ink);">'+s[1]+'</div></div>';
    }).join('');

    el.innerHTML = acts.map(function(a){
      var isBuy = a.side==='buy';
      var bg = isBuy ? 'var(--green-soft)' : 'var(--red-soft)';
      var ico = isBuy ? '🟢' : '🔴';
      var total = (a.price * a.qty).toFixed(2);
      return '<div style="background:'+bg+';border-radius:10px;padding:12px 14px;margin-bottom:8px;">'
        +'<div style="display:flex;justify-content:space-between;align-items:center;">'
        +'<span style="font-weight:700;font-size:15px;">'+ico+' '+a.symbol+'</span>'
        +'<span style="font-size:12px;color:var(--ink-2);">'+a.time+'</span>'
        +'</div>'
        +'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;font-size:12px;color:var(--ink-2);margin-top:8px;">'
        +'<div>السعر<br><b style="color:var(--ink);">$'+a.price.toFixed(2)+'</b></div>'
        +'<div>الكمية<br><b style="color:var(--ink);">'+a.qty+'</b></div>'
        +'<div>الإجمالي<br><b style="color:var(--ink);">$'+total+'</b></div>'
        +'</div></div>';
    }).join('');
  }).catch(function(){
    el.innerHTML='<div class="empty"><div class="empty-ico">⚠️</div>تعذّر جلب السجل</div>';
  });
}
...
function renderSig(s) {
  ...
  var isSA = s.is_sa === true;
  var cur = isSA ? ' ر.س' : '$';
  var curPre = isSA ? '' : '$';
  var ptp = s.ptp || (s.price>0 ? (Math.round((s.t1-s.price)/s.price*1000)/10) : 0);
  var psl = s.psl || (s.price>0 ? (Math.round((s.price-s.sl)/s.price*1000)/10) : 0);
  var trend = s.trend || '';
  var adx = s.adx || '—';
  var eta = s.eta || '—';
  var priceDisp = isSA
  ...
  return '<div class="sig">'+
    ...
    '<div class="sig-meta"><span class="mb">RSI '+s.rsi+'</span><span class="mb">ADX '+adx+'</span><span class="mb">⏱ '+eta+'</span></div></div>'+
    '<div class="sig-px">'+priceDisp+live+'</div></div>'+
    ...
    '<div class="lvl sl"><div class="lvl-lbl">وقف</div><div class="lvl-val">'+curPre+s.sl+cur+'</div><div class="lvl-pct">-'+psl+'%</div></div></div></div></div>';
}
...
def scan():
    ...
    if market not in scan_state: market="us"
    if scan_state[market]["status"]=="scanning":
        return jsonify({"status":"already_running"})
    if market=="sa":
        t=threading.Thread(target=run_scan_sa,daemon=True)
    else:
...
def api_config():
    if request.method=="POST":
        ...
        for k in d:
            if k in cfg:
                # حماية: لا نسمح بمسح المفاتيح بقيمة فارغة أو مقنّعة (من واجهة الإعدادات)
                if k in ("key","secret") and (not d[k] or str(d[k]).startswith("•")):
                    continue
                cfg[k] = d[k]
        # ── سقف أمان على حد الصفقات اليومي — يمنع خطأ إدخال (مثل 500 بدل 8) من تعطيل قافل الأمان ──
        try:
            if int(cfg.get("max_daily_trades", 8)) > 20:
                cfg["max_daily_trades"] = 20
        except (TypeError, ValueError):
            cfg["max_daily_trades"] = 8
        save_cfg(cfg)
        return jsonify({"ok":True, "max_daily_trades": cfg.get("max_daily_trades")})
    cfg = load_cfg()
    safe = {k:v for k,v in cfg.items() if k not in ("key","secret")}
    safe["has_key"] = bool(cfg.get("key"))
    # 🔒 أمان: لا نُرجع المفاتيح أبداً — الرابط عام على Render وأي شخص يعرفه كان يقدر يسرقها
    safe["key"] = "••••••••" if cfg.get("key") else ""
    safe["secret"] = "••••••••" if cfg.get("secret") else ""
    return jsonify(safe)

@app.route("/api/test-realtime")
def test_realtime():
    """
    اختبار سريع (v3.4): يتصل بـ Alpaca WebSocket اللحظي، يصادق بالمفاتيح
    المحفوظة، يشترك بسهم AAPL لثوانٍ، ويرجع تقرير نصي بالنتيجة —
    كل هذا يشتغل على سيرفر Render نفسه، بدون ما يحتاج المستخدم يثبت
    أو يشغّل أي شي على جهازه.
    """
    if websocket is None:
        return jsonify({"ok": False, "log": ["❌ مكتبة websocket-client غير مثبتة على السيرفر — تأكد من رفع requirements.txt المحدث وإعادة النشر"]})

    cfg = load_cfg()
    api_key = cfg.get("key")
    api_secret = cfg.get("secret")
    if not api_key or not api_secret:
        return jsonify({"ok": False, "log": ["❌ ما فيه مفاتيح API محفوظة بالإعدادات — احفظها أول"]})

    log = []
    result = {"authenticated": False, "subscribed": False, "got_tick": False, "error": None}

    def on_open(ws):
        log.append("🔌 الاتصال فتح — جاري المصادقة...")
    ...
    def on_message(ws, message):
        ...
        for msg in data:
            ...
            if t == "success" and msg.get("msg") == "authenticated":
                ...
                log.append("✅ المصادقة نجحت")
                ws.send(json.dumps({"action": "subscribe", "trades": ["AAPL"]}))
            elif t == "subscription":
                result["subscribed"] = True
                log.append(f"✅ الاشتراك تأكد بـ AAPL")
            elif t == "error":
                result["error"] = str(msg)
                log.append(f"❌ خطأ من Alpaca: {msg}")
                ws.close()
            elif t == "t":
                result["got_tick"] = True
                log.append(f"📈 صفقة لحظية وصلت: AAPL @ ${msg.get('p')}")
                ws.close()

    def on_error(ws, error):
        result["error"] = str(error)
        log.append(f"❌ خطأ اتصال: {error}")
    ...
    t.start()
    t.join(timeout=8)  # نمهله 8 ثواني بس
    try:
        ws.close()
    except Exception:
        pass
    ...
    if result["got_tick"]:
        log.append("🎉 الاتصال اللحظي شغال 100% — استقبلنا صفقة حقيقية")
    elif result["subscribed"]:
        log.append("✅ الاتصال والاشتراك نجحوا، بس ما وصلت صفقة خلال 8 ثواني — طبيعي لو السوق مقفل حالياً، أو جرب مرة ثانية ووقت السوق مفتوح")
    elif result["authenticated"]:
        log.append("⚠️ المصادقة نجحت بس الاشتراك ما تأكد — جرب مرة ثانية")
    elif not result["error"]:
        log.append("⚠️ ما وصل رد خلال 8 ثواني — تأكد الحساب مفعّل له صلاحية البيانات اللحظية")

    return jsonify({"ok": bool(result["authenticated"]), "log": log, "detail": result})
...
def positions():
    ...
    for p in pos:
        try:
            qty_f = float(p.get("qty",0))
            entry_f = float(p.get("avg_entry_price",0))
            cost_basis = float(p.get("cost_basis", qty_f*entry_f))
            ...
            out.append({
                ...
                "invested": round(abs(cost_basis),2),   # المبلغ الأصلي اللي دخل فيه المركز
                "value": round(abs(market_value),2),     # القيمة الحالية للمركز
            })
        except: pass
...
def trade():
    ...
    # (v3.5) إصلاح: هذا الـ endpoint كان يتجاوز كل حمايات safe_buy() تمامًا —
    # لا فحص "السوق مفتوح"، ولا فحص "اشترينا نفس السهم اليوم". هذا كان يسمح
    # بإرسال نفس أمر الشراء عدة مرات حتى والسوق مقفول (مثل عطلة نهاية الأسبوع)،
    # فيتكدّس عدة أوامر معلّقة بلا داعٍ على نفس الرمز.
    if side == "buy":
        if not is_crypto and not market_is_open():
            return jsonify({"ok":False,"msg":"السوق مغلق — ما ينرسل أمر شراء الحين"})
        today_syms = get_today_auto_buys_from_alpaca()
        if today_syms is None:
            return jsonify({"ok":False,"msg":"تعذّر التحقق من صفقات اليوم من Alpaca — أُلغي الشراء احتياطاً"})
        if symbol in today_syms:
            return jsonify({"ok":False,"msg":f"عندك أمر شراء اليوم على {symbol} أصلاً (منفّذ أو معلّق) — ما ينرسل أمر ثاني"})

    # ── منع فتح شورت بالغلط من زر البيع اليدوي ──
    # الاستراتيجية كلها (TP1/TP2/Trailing) مبنية على مراكز شراء فقط.
    # لو "بيع" بكمية أكبر من اللي عندك فعلاً، Alpaca تفتح شورت تلقائي = خطر غير مراقب.
    if side == "sell":
        norm_sym = symbol.replace("-USD","/USD") if is_crypto else symbol
        held_qty = 0.0
        for pos in api_positions():
            psym = pos.get("symbol","").replace("/USD","-USD")
            if psym == symbol.replace("/USD","-USD"):
                try: held_qty = float(pos.get("qty",0))
                except: held_qty = 0.0
                break
        try: req_qty = float(qty)
        except: req_qty = 0.0
        if held_qty <= 0:
            return jsonify({"ok":False,"msg":f"ما عندك مركز شراء في {symbol} — البيع هذا رح يفتح شورت، وهو غير مدعوم بالبوت (ممنوع تلقائياً)"})
        if req_qty > held_qty:
            return jsonify({"ok":False,"msg":f"طلبت بيع {req_qty} بس عندك {held_qty} بس — قللت الكمية تلقائياً لأقصى حد متاح"}), 400
...
@app.route("/api/alpaca_history")
def alpaca_trades_history_route():
    """تاريخ الصفقات من Alpaca مباشرة — لا يضيع عند deploy"""
    activities = api_activities(200)
    out = []
    for a in activities:
        try:
            side = a.get("side","")
            sym  = a.get("symbol","").replace("/USD","-USD")
            qty  = float(a.get("qty") or a.get("cumulative_qty") or 0)
            price= float(a.get("price") or 0)
            t    = a.get("transaction_time","")[:16].replace("T"," ")
            out.append({
                "symbol": sym, "side": side, "qty": qty,
                "price": price, "time": t,
                "order_id": a.get("order_id",""),
                "type": a.get("type","fill"),
            })
        except: pass
...
def export_excel():
    """تصدير كل نشاطات Alpaca بصيغة CSV متوافقة مع Excel"""
    ...
    for a in activities:
        try:
            ...
            # ── إصلاح: Alpaca ترجع الوقت بحقل transaction_time مو time — كان يطلع فاضي دايماً ──
            ...
            if raw_time:
                try:
                    ...
                    time_str = raw_time
            writer.writerow([
                a.get("symbol",""), a.get("side",""),
                qty, price, total,
                time_str, a.get("order_id","")
            ])
        except: pass
    ...
    return Response(
        csv_bytes,
        mimetype="text/csv",
        headers={"Content-Disposition": "attachment;filename=jalal_radar_history.csv"}
    )

@app.route("/api/export_internal_log")
def export_internal_log():
    """
    ...
    return Response(
        csv_bytes,
        mimetype="text/csv",
        headers={"Content-Disposition": "attachment;filename=jalal_radar_internal_log.csv"}
    )

# ════════════════════════════════════════════════════════════
# (v3.4) الاختبار التاريخي (Backtest) — يشتغل بالخلفية، يخزّن النتيجة بالذاكرة
# ════════════════════════════════════════════════════════════
_backtest_state = {"running": False, "progress": 0, "total": 0, "result": None, "error": None}
...
def _run_backtest_job():
    ...
    try:
        ...
        _backtest_state["total"] = len(symbols)
        for idx, sym in enumerate(symbols):
            _backtest_state["progress"] = idx + 1
            try:
                df = get_history(sym, "2y", "1d")
                trades = backtest_symbol(df, sym, capital=100000)
                all_trades.extend(trades)
            except Exception as e:
                log(f"⚠️ [Backtest] فشل {sym}: {e}")
        report = summarize(all_trades)
        report["raw_trades"] = all_trades
...
def backtest_start():
    if backtest_symbol is None:
        return jsonify({"ok": False, "msg": "backtest_engine.py غير موجود على السيرفر — ارفعه أول"})
    if _backtest_state["running"]:
        return jsonify({"ok": False, "msg": "فيه اختبار شغال حالياً، استنى يخلص"})
    _backtest_state.update({"running": True, "progress": 0, "total": 0, "result": None, "error": None})
    threading.Thread(target=_run_backtest_job, daemon=True).start()
    return jsonify({"ok": True, "msg": "بدأ الاختبار — راقب التقدم عبر /api/backtest/status"})
...
def stats():
    """
    (v3.3) إحصائيات من Alpaca مباشرة — دقيقة ولا تضيع عند deploy.
    الصفقة المقفلة = دورة كاملة (round trip): من أول شراء حتى تصفية المركز.
    البيع الجزئي (TP1/TP2/وقف متحرك) ما يُعدّ صفقات منفصلة.
    """
    acts = api_activities(500)
    acts.sort(key=lambda a: a.get("transaction_time", ""))

    open_pos = {}   # symbol -> {"qty":, "cost":, "realized":}
    closed = []     # صفقات مقفلة (دورات كاملة)
    for a in acts:
        try:
            sym   = a.get("symbol", "")
            side  = a.get("side", "")
            ...
            if not sym or qty <= 0 or price <= 0:
                continue
            p = open_pos.setdefault(sym, {"qty": 0.0, "cost": 0.0, "realized": 0.0})
            if side == "buy":
                p["qty"]  += qty
                p["cost"] += qty * price
            elif side.startswith("sell") and p["qty"] > 1e-9:
                avg = p["cost"] / p["qty"]
                sold = min(qty, p["qty"])
                p["realized"] += (price - avg) * sold
                p["qty"]  -= sold
                p["cost"] -= avg * sold
                if p["qty"] <= 1e-6:  # المركز تصفّى — دورة مقفلة
                    closed.append({"symbol": sym,
                                   "pnl": round(p["realized"], 2),
                                   "time": a.get("transaction_time", "")[:16]})
                    open_pos[sym] = {"qty": 0.0, "cost": 0.0, "realized": 0.0}
        except Exception:
            continue
    ...
    # عدّاد اليوم من سجل Alpaca الدائم (v3.4) — بدل الملف المحلي اللي ينمسح مع كل رفعة كود
    today_buy_symbols = get_today_auto_buys_from_alpaca() or []
    cfg = load_cfg()
    return jsonify({
        "total": len(closed) + sum(1 for p in open_pos.values() if p["qty"] > 1e-6),
        "closed": len(closed), "wins": len(wins), "losses": len(losses),
        "win_rate": wr, "total_pnl": total_pnl,
        "today": len(today_buy_symbols),
        "max_daily": int(cfg.get("max_daily_trades", rm.MAX_TRADES_PER_DAY)),
    })
...
# ════════════════════════════════════════════════════════════
# فحص ذاتي عند التشغيل — يكشف فوراً إذا بيانات Alpaca شغالة
# ════════════════════════════════════════════════════════════
def startup_selftest():
    try:
        _cfg = load_cfg()
        log(f"🔬 [SELFTEST] المفتاح موجود: {bool(_cfg.get('key'))} | data_url: {_cfg.get('data_url')}")
        _df = alpaca_history("AAPL")
        if not _df.empty:
            log(f"✅ [SELFTEST] بيانات Alpaca تعمل! AAPL: {len(_df)} شمعة، آخر إغلاق {_df['Close'].iloc[-1]:.2f}")
        else:
            log("🔴 [SELFTEST] بيانات Alpaca فشلت لـ AAPL — راجع رسالة ALPACA-ERR أعلاه لمعرفة السبب")
    except Exception as _e:
        log(f"🔴 [SELFTEST] استثناء: {type(_e).__name__}: {_e}")
...
# ── (v3.4) المراقبة اللحظية للخروج — طبقة اختيارية فوق الفحص الدوري ──
# لو انقطعت أو فشلت لأي سبب، الفحص الدوري كل 15 دقيقة يستمر كخط دفاع أساسي.
_realtime_monitor = None
try:
    from realtime_monitor import RealtimeExitMonitor

    def _get_held_symbols():
        try:
            return [
                p.get("symbol", "").replace("/USD", "-USD")
    ...
    def _rt_status_log(status, detail):
        print(f"[realtime-status] {detail}")

    _cfg_startup = load_cfg()
    if _cfg_startup.get("key") and _cfg_startup.get("secret") and _cfg_startup.get("enabled"):
        _realtime_monitor = RealtimeExitMonitor(
            api_key=_cfg_startup.get("key"),
            api_secret=_cfg_startup.get("secret"),
            on_tick_callback=realtime_on_tick,
            get_held_symbols_callback=_get_held_symbols,
            on_status_change=_rt_status_log,
        )
        _realtime_monitor.start()
        print("⚡ المراقبة اللحظية (v3.4) بدأت — تراقب المراكز المفتوحة فقط")
    else:
