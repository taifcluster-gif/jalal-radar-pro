...
def safe_buy(signal, source="auto"):
    ...
    # 3. حد الصفقات اليومي
    if source=="auto" and len(today_buys) >= int(cfg.get("max_daily_trades", rm.MAX_TRADES_PER_DAY)):
    ...
    if source == "auto" and not is_crypto:
        ...
        _sector_count = sum(1 for t in today_buys if rm.get_sector(t.get("symbol","")) == _sector)
...
def stats():
    ...
    # عدّاد اليوم من الملف المحلي — نفس مصدر تطبيق الحد في safe_buy عشان يتطابقون
    trades = load_trades()
    today = datetime.now().strftime("%Y-%m-%d")
    today_buys = [t for t in trades if t.get("time","").startswith(today)
                  and t.get("side")=="buy" and t.get("source")=="auto"]
