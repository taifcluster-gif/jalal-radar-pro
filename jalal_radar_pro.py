...
def safe_buy(signal, source="auto"):
    ...
    if is_crypto:
        qty = round(max_usd / price, 6)  # كريبتو كسور
    else:
        qty = max(1, int(max_usd / price))

    # نرسل الأمر — limit بسعر الدخول المقترح
...
def scheduler_loop():
    ...
    1. تراقب المراكز (بيع ذكي)
    2. تلغي الأوامر المعلّقة القديمة
    3. عند فتح السوق (من Alpaca) → مسح وتداول مرة وحدة
    4. الكريبتو 24/7 → مسح كل ساعة لو مفعّل
    """
    ...
    while True:
        try:
            ...
            if cfg.get("enabled") and cfg.get("key"):
                ...
                # 3. السوق الأمريكي — نعتمد على ساعة Alpaca الحقيقية
                if "us" in cfg.get("markets",["us"]):
                    clock = api_clock()
                    if "error" not in clock and clock.get("is_open"):
                        # السوق مفتوح فعلاً (Alpaca أكّد) ولم نمسح اليوم
                        if last_us_scan_day != today:
                            res = auto_scan_and_trade("us")
                            if scan_state["us"].get("data"):
                                last_us_scan_day = today
                                print(f"✅ مسح أمريكي تلقائي {today} — {res} صفقة")
...
.sig-meta { font-size:12px; color:var(--ink-2); margin-top:2px; }
...
.tag { background:var(--surface-2); border:1px solid var(--line); border-radius:8px; padding:4px 9px; font-size:11.5px; color:var(--ink-2); }
.tag b { color:var(--ink); }
...
.lvl-val { font-size:14px; font-weight:700; margin-top:2px; }
...
.fav-star { cursor:pointer; font-size:16px; color:var(--ink-3); margin-right:7px; transition:.15s; }
...
function renderSig(s) {
  ...
  return '<div class="sig">'+
    ...
    '<div class="sig-meta">RSI '+s.rsi+' · ADX '+s.adx+' · ⏱ '+(s.eta||'—')+'</div></div>'+
